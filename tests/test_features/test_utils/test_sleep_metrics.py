...
from cosinorage.features.utils.sleep_metrics import (
    NWB, PTA, SOL, SRI, TST, WASO, apply_sleep_wake_predictions)
...
def sample_data():
    ...
    dates = pd.date_range(
        start="2024-01-01", end="2024-01-02 23:59:00", freq="1min"
    )
...
def test_tst(sleep_data):
    ...
    assert all(isinstance(x, int) for x in result)
    assert all(
        0 <= x <= 1440 for x in result
    )  # TST should be between 0 and 1440 minutes
...
def test_sri_insufficient_data():
    ...
    dates = pd.date_range(
        start="2024-01-01", end="2024-01-01 23:59:00", freq="1min"
    )
    df = pd.DataFrame(index=dates)
    df["sleep"] = 0
