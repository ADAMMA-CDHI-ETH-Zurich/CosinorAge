...
def create_test_data(days=1, amplitude=1, mesor=0, phase_shift=0):
    ...
    minutes = 1440 * days
    timestamps = [
        datetime(2024, 1, 1) + timedelta(minutes=i) for i in range(minutes)
    ]
...
def test_cosinor_multiday_phase_shift():
    ...
    test_df = create_test_data(
        days=1, amplitude=1, mesor=0, phase_shift=phase_shift
    )
    ...
    assert np.isclose(
        params["acrophase_time"], expected_acrophase_time, atol=30
...
def test_invalid_input_wrong_length():
    ...
    with pytest.raises(
        ValueError, match="Data length is not a multiple of a day"
    ):
        cosinor_multiday(df)
...
def test_cosinor_multiday_output_types():
    ...
    assert all(
        key in params
        for key in ["mesor", "amplitude", "acrophase", "acrophase_time"]
    )
