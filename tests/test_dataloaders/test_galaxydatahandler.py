...
class TestGalaxyDataHandler:
    ...
    def sample_csv_data(self):
        ...
        data = pd.DataFrame(
            {"time": dates, "enmo_mg": np.random.randn(100) * 10 + 50}
        )
        return data
    ...
    def test_init_defaults(self):
        ...
        with patch("os.path.isdir", return_value=True):
            with patch.object(
                GalaxyDataHandler, "_GalaxyDataHandler__load_data"
            ):
                ...
                assert handler.data_columns == [
                    "acceleration_x",
                    "acceleration_y",
                    "acceleration_z",
                ]
                assert (
                    handler.meta_dict["datasource"]
        ...
        with patch("os.path.isfile", return_value=True):
            with patch.object(
                ...
                handler = GalaxyDataHandler(
                    galaxy_file_path="/dummy/file.csv", data_format="csv"
                )
    ...
    def test_init_invalid_format(self):
        ...
        with pytest.raises(
            ValueError, match="data_format must be either 'csv' or 'binary'"
        ):
            GalaxyDataHandler(
                galaxy_file_path="/dummy/path", data_format="invalid"
            )
    ...
    def test_init_invalid_type(self):
        ...
        with patch("os.path.isdir", return_value=True):
            with pytest.raises(
                ValueError,
                match="data_type must be either 'enmo' or 'accelerometer'",
            ):
                GalaxyDataHandler(
                    galaxy_file_path="/dummy/path", data_type="invalid"
                )
    ...
    def test_init_csv_with_accelerometer(self):
        ...
        with patch("os.path.isfile", return_value=True):
            with pytest.raises(
                ValueError,
                match="CSV format currently only supports 'enmo' data_type",
            ):
                GalaxyDataHandler(
    ...
    def test_init_binary_with_enmo(self):
        ...
        with patch("os.path.isdir", return_value=True):
            with pytest.raises(
                ValueError,
                match="Binary format currently only supports 'accelerometer' data_type",
            ):
                GalaxyDataHandler(
                    galaxy_file_path="/dummy/path",
                    data_format="binary",
    ...
    def test_init_invalid_enmo_columns(self):
        ...
        with patch("os.path.isfile", return_value=True):
            with pytest.raises(
                ValueError,
                match="For 'enmo' data_type, data_columns should contain exactly one column name",
            ):
                GalaxyDataHandler(
                    galaxy_file_path="/dummy/file.csv",
                    data_format="csv",
    ...
    def test_init_invalid_accelerometer_columns(self):
        ...
        with patch("os.path.isdir", return_value=True):
            with pytest.raises(
                ValueError,
                match="For 'accelerometer' data_type, data_columns should contain exactly three column names",
            ):
                GalaxyDataHandler(
                    galaxy_file_path="/dummy/path", data_columns=["x", "y"]
                )
    ...
    def test_init_csv_file_not_found(self):
        ...
        with patch("os.path.isfile", return_value=False):
            with pytest.raises(
                ValueError,
                match="For CSV format, galaxy_file_path should be a file path",
            ):
                GalaxyDataHandler(
                    galaxy_file_path="/dummy/file.csv", data_format="csv"
                )
    ...
    def test_init_binary_dir_not_found(self):
        ...
        with patch("os.path.isdir", return_value=False):
            with pytest.raises(
                ValueError,
                match="For binary format, galaxy_file_path should be a directory path",
            ):
    ...
    @patch(
        "cosinorage.datahandlers.utils.galaxy_csv.preprocess_galaxy_csv_data"
    )
    @patch(
        "cosinorage.datahandlers.utils.calc_enmo.calculate_minute_level_enmo"
    )
    ...
    @patch(
        "cosinorage.datahandlers.galaxydatahandler.filter_galaxy_binary_data"
    )
    @patch(
        "cosinorage.datahandlers.galaxydatahandler.resample_galaxy_binary_data"
    )
    @patch(
        "cosinorage.datahandlers.galaxydatahandler.preprocess_galaxy_binary_data"
    )
    @patch(
        "cosinorage.datahandlers.galaxydatahandler.calculate_minute_level_enmo"
    )
    @patch("cosinorage.datahandlers.utils.acceleration_data_to_dataframe")
    @patch(
        "cosinorage.datahandlers.utils.galaxy_binary.load_acceleration_data"
    )
    @patch("os.path.isdir")
    @patch("os.listdir")
    def test_load_binary_data(
        ...
        mock_listdir.side_effect = [
            ...
            [
                "acceleration_data1.binary"
            ],  # Second call returns files in subdirectory
        ]
        mock_isdir.side_effect = (
            lambda path: path == "/dummy/path" or path.endswith("day1")
        )
        ...
        mock_accel_data.data = pd.DataFrame(
            {
                "unix_timestamp_in_ms": pd.date_range(
                    "2024-01-01", periods=4 * 1440, freq="1min"
                ).astype(int)
                // 10**6,
        ...
        processed_df["timestamp"] = pd.to_datetime(
            processed_df["timestamp"], unit="ms"
        )
        processed_df.set_index("timestamp", inplace=True)
        processed_df.drop(
            columns=["effective_time_frame", "sensor_body_location"],
            inplace=True,
        )
    ...
    def test_unsupported_combination(self):
        ...
        with patch("os.path.isdir", return_value=True):
            with patch.object(
                ...
                with pytest.raises(
                    ValueError, match="Unsupported combination"
                ):
                    GalaxyDataHandler(galaxy_file_path="/dummy/path")
