...
from cosinorage.datahandlers.utils.ukb import (filter_ukb_data, read_ukb_data,
                                               resample_ukb_data)
...
def sample_enmo_data(tmp_path):
    ...
    header = (
        "acceleration data from 2020-01-01 00:00:00 to 2020-01-07 23:59:00"
    )
    data = pd.DataFrame(
        {
            ...
            "enmo_mg": [header]
            + [
                str(x * 100) for x in range(1439)
            ],  # Sample ENMO values as strings
        }
    )
