...
from cosinorage.datahandlers.utils.galaxy_binary import (
    acceleration_data_to_dataframe, calc_weartime, calibrate_accelerometer,
    detect_wear_periods, filter_galaxy_binary_data,
    preprocess_galaxy_binary_data, read_galaxy_binary_data, remove_noise,
...
def sample_acc_data():
    ...
    df = pd.DataFrame(
        {
            "x": np.sin(2 * np.pi * 0.5 * t) * 0.1 * gravity
            + np.random.normal(0, 0.01 * gravity, n_samples),
            "y": np.cos(2 * np.pi * 0.5 * t) * 0.1 * gravity
            + np.random.normal(0, 0.01 * gravity, n_samples),
...
def test_read_galaxy_binary_data(tmp_path):
    ...
    with patch(
        ...
        with patch(
            "cosinorage.datahandlers.utils.galaxy_binary.acceleration_data_to_dataframe"
        ) as mock_convert:
            ...
            meta_dict = {}
            result = read_galaxy_binary_data(
                str(tmp_path) + "/", meta_dict, verbose=True
            )
...
def test_filter_galaxy_binary_data(sample_acc_data):
    ...
    result = filter_galaxy_binary_data(
        sample_acc_data, meta_dict, verbose=True
    )
...
def test_resample_galaxy_binary_data(sample_acc_data):
    ...
    meta_dict = {}
    result = resample_galaxy_binary_data(
        sample_acc_data, meta_dict, verbose=True
    )
    ...
    time_diffs = (
        np.diff(result.index.astype(np.int64)) / 1e6
    )  # Convert to milliseconds
    assert np.allclose(time_diffs, 40, atol=1)


def test_preprocess_galaxy_binary_data(
    sample_acc_data, mock_calibrator, mock_wear_detector
):
    ...
    with patch(
        "cosinorage.datahandlers.utils.calibration.CalibrateAccelerometer",
        return_value=mock_calibrator,
    ):
        with patch(
            "cosinorage.datahandlers.utils.wear_detection.AccelThresholdWearDetection",
            return_value=mock_wear_detector,
        ):
            result = preprocess_galaxy_binary_data(
                sample_acc_data, preprocess_args, meta_dict, verbose=True
            )
...
def test_calibrate(sample_acc_data, mock_calibrator):
    ...
    with patch(
        ...
        result = calibrate_accelerometer(
            sample_acc_data,
            sphere_crit=1,
            sd_criteria=0.3,
            meta_dict=meta_dict,
...
def test_calc_weartime(sample_acc_data):
    ...
    sample_acc_data["wear"] = np.random.choice(
        [0, 1], size=len(sample_acc_data)
    )
