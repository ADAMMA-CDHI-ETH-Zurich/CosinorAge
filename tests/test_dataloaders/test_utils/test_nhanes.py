...
from cosinorage.datahandlers.utils.nhanes import (
    calculate_measure_time, clean_data, filter_and_preprocess_nhanes_data,
    read_nhanes_data, remove_bytes, resample_nhanes_data)
...
def mock_sas_data():
    ...
    day_numbers = np.repeat(
        range(1, 5), 24 * 60
    ).tolist()  # 4 days numbered 1-4
    ssnmp_values = list(
        range(0, n_minutes * 80, 80)
    )  # 80 measurements per second
    ...
    min_data = pd.DataFrame(
        {
            ...
            "PAXPREDM": [1]
            * n_minutes,  # Valid wear/non-sleep (not in [3, 4])
...
def test_remove_bytes():
    # Test data
    df = pd.DataFrame({"text_col": [b"hello", b"world"], "num_col": [1, 2]})
...
def test_filter_and_preprocess_nhanes_data(sample_nhanes_df):
    meta_dict = {}
    result = filter_and_preprocess_nhanes_data(
        sample_nhanes_df, meta_dict, verbose=True
    )
...
def test_resample_nhanes_data(sample_nhanes_df):
    ...
    assert (
        result.index[0].strftime("%Y-%m-%d %H:%M:%S") == "2023-01-01 00:00:00"
    )
    assert (
        result.index[-1].strftime("%Y-%m-%d %H:%M:%S") == "2023-01-05 23:58:00"
    )


def test_read_nhanes_data_missing_seqn():
    with pytest.raises(
        ValueError, match="The seqn is required for nhanes data"
    ):
