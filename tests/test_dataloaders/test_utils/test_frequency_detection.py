...
class TestFrequencyDetection:
    ...
    def test_detect_frequency_regular_25hz(self):
        ...
        timestamps = pd.Series([base_time + pd.Timedelta(milliseconds=40*i) for i in range(100)])
    ...
    def test_detect_frequency_regular_1hz(self):
        ...
        base_time = pd.Timestamp('2024-01-01 00:00:00')
        timestamps = pd.Series([base_time + pd.Timedelta(seconds=i) for i in range(100)])
    ...
    def test_detect_frequency_with_irregular_samples(self):
        ...
        for i in range(100):
            if i % 20 == 0:  # Every 20th sample has irregular timing
                timestamps.append(base_time + pd.Timedelta(milliseconds=40*i + 100))
            else:
    ...
    def test_detect_frequency_mixed_frequencies(self):
        ...
        for i in range(40):
            timestamps.append(base_time + pd.Timedelta(milliseconds=2400 + 100*i))
    ...
    def test_detect_frequency_nan_values(self):
        ...
        base_time = pd.Timestamp('2024-01-01 00:00:00')
        timestamps = pd.Series([
            base_time + pd.Timedelta(milliseconds=40*i) if i % 10 != 0 else pd.NaT 
            for i in range(100)
        ])
